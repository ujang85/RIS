...
use app\models\HasilPasienSearch;
...
/**
 * RegPasien2Controller implements the CRUD actions for RegPasien model.
 */
class RegPasien2Controller extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {    
        $searchModel = new HasilPasienSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    ...
    {   /*
        $hasil= Yii::$app->db->createCommand("SELECT GROUP_CONCAT(jenis_tindakan.nama_tarif SEPARATOR ', ') AS tarif
        ...
        FROM input_periksa LEFT JOIN jenis_tindakan
        ON input_periksa.id_jenis_tindakan=jenis_tindakan.id
        WHERE input_periksa.id=:id GROUP BY input_periksa.id;")
        ->bindValue(':id', $id)
        ->queryScalar();
        return $hasil;
    }
    ...
    protected function findDetail($id)
    {
       ...
       $detailModel = new InputPeriksaSearch();
    return $detailModel->search(['InputPeriksaSearch'=>['id'=>$id]]);
    }
    public function actionCetak($id)
    {
       
     //  return $this->render('cetak_hasil', [
        return $this->render('hasil_radiologi', [
        'model' => $this->findMaster($id),
        'detail' => $this->findDetail($id),
        'hasil' => $this->findHasil($id),
    ]);
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> "Create new RegPasien",
                    'content'=>'<span class="text-success">Create RegPasien success</span>',
                    'footer'=> Html::button('Close',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
    ...
    public function actionUpdate($id)
    {
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                return [
                    'title'=> "Input Kesan pasien: ".$model->no_cm,
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'id' => $model->id]);
               // return $this->redirect(['in', 'id' => $model->id]);
            } else {
    ...
    /**
     * Delete an existing RegPasien model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing RegPasien model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
